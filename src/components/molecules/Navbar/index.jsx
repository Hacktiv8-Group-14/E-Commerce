    ...
    return(
        <nav className="w-full font-bold text-white bg-[#242582] p-2 sm:p-4 flex justify-between flex-col sm:flex-row fixed z-10 top-0">
            <div className="flex flex-row justify-between mb-2 sm:mb-0">
                {/* Nav Brand(?)/Header */}
                <h1 className="text-2xl sm:text-3xl">BukaPedia</h1>
            ...
            <div className={`text-lg sm:text-xl flex-col sm:flex-row sm:items-center ${isNavExpanded ? 'flex' : 'hidden sm:flex'}`}>
                {/* User Navlink */}
                <Link to="/" className="hover:text-[#F64C72] transition" onClick={() => setIsNavExpanded(false)}>
                    Products
                </Link>
                <Link to="/cart" className="mx-0 sm:mx-16 my-3 sm:my-0 hover:text-[#F64C72] transition" onClick={() => setIsNavExpanded(false)}>
                    Cart
                </Link>
                ...
                {/* Login/Logout Button */}
                <button className="border-[#F64C72] hover:border-white border-2 rounded-lg px-5 py-1 text-base sm:text-lg text-[#F64C72] hover:text-white transition">
                    Login
                </button>
            </div>
        </nav>
